...
begin
  ...
  Value := StringReplace(Value, '-', '', [rfReplaceAll]); // remove o ponto dos milhares
  //Value := StringReplace(Value, ',', '.', [rfReplaceAll]); // troca a vírgula por ponto para decimal
...
{        if (gravar.Strings[5] <> 'Valor Liquidado (R$)')
              and (gravar.Strings[5]  <> 'Saldo até o vencimento')
               and (gravar.Strings[5]  <> 'Aluguel')
               and (gravar.Strings[5]  <> 'LANÇAMENTO')
...
begin
  ...
  try
    ...
    RowList.Delimiter := ';'; // Defina o delimitador aqui, como vírgula
    RowList.StrictDelimiter := True; // Trata o delimitador estritamente (não trata espaços como delimitadores)
    ...
    begin
      ...
      // Agora, RowList contém cada valor de coluna da linha atual
      // Aqui, você pode processar cada valor de coluna como necessário
      ...
      begin
          ...
          aCli.Append;
           aCli.FieldByName('CODIGO_CENTRALIZADOR').AsString:= RowList.Strings[0];
             ...
             aCli.FieldByName('DATA_VENDA').AsString:= copy(RowList.Strings[3],1, SpacePos - 1);// Obtém o primeiro valor de coluna
        // Faça algo com ColumnValue, como adicionar a um grid, listbox, etc.
              cod_interno:= cod_interno+1;
            aCli.FieldByName('cod_interno').AsInteger:= cod_interno;
        aCli.Post;
      end;
